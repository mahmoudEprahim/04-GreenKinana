...
//                return session_lang(\App\receiptsType::where('invoice_id',$query->invoice_id)->first()->name_en,\App\receiptsType::where('invoice_id',$query->invoice_id)->first()->name_ar) ;
                $glcc= \App\glcc::where('id',\App\receiptsType::where('invoice_id',$query->invoice_id)->first()['cc_id'])->first()['name_ar'];
                $department = session_lang(\App\receiptsType::where('invoice_id',$query->invoice_id)->first()['name_ar'],\App\receiptsType::where('invoice_id',$query->invoice_id)->first()['name_ar']);
                return    !empty($glcc) ? $department .'/'. $glcc : $department;
...
//        return receipts::query()->orderByDesc('created_at')->where('status',1)->whereIn('receiptsType_id',[3,4]);
        ...
        return receipts::query()->where('status',1)->whereIn('receiptsType_id',[3,4]);
    }
